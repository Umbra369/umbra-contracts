...
contract UmbraRouterV3FeeTest is Test {
    ...
    function _params(uint256 amt, uint256 minOut, uint256 surplusFloor, uint256 nonce, bytes memory sig)
        internal
        view
        returns (UmbraRouterV3.ExecuteParams memory p)
    {
    ...
    function _sign(uint256 pk, UmbraRouterV3.ExecuteParams memory p) internal view returns (bytes memory) {
    ...
    function test_no_fee_when_no_surplus() public {
        ...
        UmbraRouterV3.ExecuteParams memory p = _params(amt, 0, expected, _nextNonce(), "");
    ...
    function test_cap_binds_on_large_surplus() public {
        ...
        UmbraRouterV3.ExecuteParams memory p = _params(amt, 0, 1, _nextNonce(), "");
        p.umbraSig = _sign(signerPk, p);
    ...
    function test_fee_is_half_surplus_when_under_cap() public {
        ...
        UmbraRouterV3.ExecuteParams memory p = _params(amt, 0, floor, _nextNonce(), "");
        p.umbraSig = _sign(signerPk, p);
    ...
    function test_net_below_minAmountOut_reverts() public {
        ...
        UmbraRouterV3.ExecuteParams memory p = _params(amt, expected - cap + 1, 1, _nextNonce(), "");
        p.umbraSig = _sign(signerPk, p);

        vm.expectRevert(UmbraRouterV3.InsufficientOutput.selector);
        router.execute(p);
    }
    ...
    function test_bad_signature_reverts() public {
        ...
        vm.expectRevert(UmbraRouterV3.BadSig.selector);
        router.execute(p);
    }
    ...
    function test_malformed_signature_reverts() public {
        ...
        UmbraRouterV3.ExecuteParams memory p = _params(amt, 0, 1, _nextNonce(), hex"deadbeef"); // 4 bytes, not 65
        vm.expectRevert(UmbraRouterV3.BadSig.selector);
        router.execute(p);
    }
    ...
    function test_fee_disabled_skips_verification() public {
        ...
        UmbraRouterV3.ExecuteParams memory p = _params(amt, 0, 12345, 0, "");
    ...
    function test_tally_increments_by_signed_fields() public {
        ...
        UmbraRouterV3.ExecuteParams memory p = _params(amt, 0, 0, _nextNonce(), "");
        p.plsRate = plsRate;
    ...
    function test_flat_fee_when_surplus_floor_zero() public {
        ...
        UmbraRouterV3.ExecuteParams memory p = _params(amt, 0, 0, _nextNonce(), ""); // surplusFloor = 0
        p.umbraSig = _sign(signerPk, p);
    ...
    function test_distinct_nonces_both_succeed() public {
        ...
        UmbraRouterV3.ExecuteParams memory pa = _params(amt, 0, 1, _nextNonce(), "");
        ...
        UmbraRouterV3.ExecuteParams memory pb = _params(amt, 0, 1, _nextNonce(), "");
