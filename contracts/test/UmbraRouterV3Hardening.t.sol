...
/// Hardening tests for UmbraRouterV3 v4 deltas.
...
contract UmbraRouterV3HardeningTest is Test {
    ...
    function _buildParams(uint256 amt, uint256 floor, uint256 nonce, bytes memory sig)
        internal view returns (UmbraRouterV3.ExecuteParams memory p)
    {
    ...
    function _sign8(uint256 pk, UmbraRouterV3.ExecuteParams memory p) internal view returns (bytes memory) {
    ...
    //
    // COMPILE RED: UmbraRouterV3.BadV2Pool does not exist yet → compile error.
    // ────────────────────────────────────────────────────────────────────────
    function test_bad_v2_pool_reverts_on_mismatch() public {
        ...
        UmbraRouterV3.Path[] memory paths = new UmbraRouterV3.Path[](1);
        paths[0] = UmbraRouterV3.Path({inputBps: 10_000, hops: hops});
        UmbraRouterV3.ExecuteParams memory p = UmbraRouterV3.ExecuteParams({
            tokenIn: WPLS, tokenOut: DAI, amountIn: amt,
        ...
        vm.expectRevert(UmbraRouterV3.BadV2Pool.selector);
        router.execute(p);
    }
    ...
    function test_8field_sig_accepted_after_typehash_upgrade() public {
        ...
        _fund(amt);
        UmbraRouterV3.ExecuteParams memory p = _buildParams(amt, 0, 42, "");
    ...
    function test_return_value_equals_recipient_delta_non_fot() public {
        ...
        uint256 amt = 1_000_000e18;
        _fund(amt);
        UmbraRouterV3.ExecuteParams memory p = _buildParams(amt, 0, 0, "");
        p.umbraSig = "";
