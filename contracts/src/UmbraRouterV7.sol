...
contract UmbraRouterV7 is ReentrancyGuard {
    ...
    address public feeRecipient; // Treasury (v7: set one-step via setFeeRecipient)
    ...
    /// v7: emitted on a one-step feeRecipient change (owner only).
    event FeeRecipientSet(address indexed recipient);
    /// v7: emitted when an Algebra factory is appended; `forkId` is its new index.
    event AlgebraFactoryAdded(uint256 indexed forkId, address indexed factory);
    ...
    /// v7: append an Algebra factory — the new fork's id is the current array length. Lets a
    /// new Algebra-based DEX (e.g. a future SwitchX generation) be wired in WITHOUT a router
    /// redeploy — the fix for v6, whose algebra set was constructor-only. The engine's
    /// `ALGEBRA_DEXES` order MUST stay in lockstep with this array (index == forkId). Append
    /// only (never mutates an existing forkId, so live routes can't be repointed). Mirrors
    /// setVault/setStablePool/setV3Router; owner is trusted (use a multisig).
    function addAlgebraFactory(address factory) external onlyOwner {
        if (factory == address(0)) revert BadAlgebraPool();
        emit AlgebraFactoryAdded(algebraFactories.length, factory);
        algebraFactories.push(factory);
    }
    ...
    /// v7: one-step feeRecipient set (owner only). Fees stream directly to `feeRecipient`
    /// on every swap (never accrued), so a zero address is rejected — a fat-finger can't
    /// silently burn revenue to 0x0 — and every change is logged for monitoring. The owner
    /// is trusted (use a multisig); this replaces v6's two-step propose/accept by design.
    function setFeeRecipient(address addr) external onlyOwner {
        if (addr == address(0)) revert BadFeeConfig();
        feeRecipient = addr;
        emit FeeRecipientSet(addr);
    }
