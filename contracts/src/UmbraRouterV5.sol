...
/// @title UmbraRouterV5 — executes engine split routes across V2/V3/Balancer pools.
...
contract UmbraRouterV5 is ReentrancyGuard {
    ...
    function execute(ExecuteParams calldata p) external payable nonReentrant returns (uint256) {
        ...
        // the input below never disturbs this snapshot.
        ...
        if (p.flags & FIRST_HOP_DIRECT != 0) {
            ...
            _pullInput(p, tokenInErc, pair); // user -> pair directly (ONE tax)
            uint256 firstOut = _swapV2Funded(h0, pair, pairBefore);
            _runHopsFrom(path, 1, h0.tokenOut, firstOut); // remaining hops run router-funded
        } else {
            ...
            if (nativeIn) {
                ...
                _pullInput(p, tokenInErc, address(this));
            }
    ...
    function _pullInput(ExecuteParams calldata p, address token, address to) private {
        if (p.flags & USE_PERMIT2 != 0) {
            ...
            IPermit2(PERMIT2).permitTransferFrom(
                permit,
                IPermit2.SignatureTransferDetails({to: to, requestedAmount: p.amountIn}),
                msg.sender,
                sig
            );
        } else {
            token.safeTransferFrom(msg.sender, to, p.amountIn);
        }
    }
    ...
    function _runPath(Path calldata path, address tokenIn0, uint256 amount0) private {
        _runHopsFrom(path, 0, tokenIn0, amount0);
    }
    ...
    /// `_swapV2Funded` after a direct user->pair transfer.
    function _runHopsFrom(Path calldata path, uint256 start, address tokenIn0, uint256 amount0) private {
    ...
    function _swapV2Funded(Hop calldata hop, address pair, uint256 pairBefore) private returns (uint256) {
        uint256 feeNum = (hop.pool >> 160) & 0xFFFFFF;
        if (feeNum == 0) feeNum = 997000;
        address token0 = IUniV2Pair(pair).token0();
        address token1 = IUniV2Pair(pair).token1();
